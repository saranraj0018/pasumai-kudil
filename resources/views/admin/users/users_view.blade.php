        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-gradient-to-b from-[#CAF1DE] to-[#E1F8DC] border border-gray-200 shadow-md rounded-2xl p-6">
                ...
                <div class="space-y-2 text-gray-700">
                    ...
                    <p class="font-semibold text-red-700"><span class="font-medium">Valid date:</span> {{ $getuserSubscription->valid_date ?? '—' }}</p>
                </div>
                <div class="mt-6">
                ...
                <div class="flex items-center justify-between mb-5">
                    <div>
                        ...
                        <p class="text-gray-700 mt-1 font-semibold">
                            Current Wallet Balance: <span class="font-semibold text-green-600">₹ {{ $user->get_wallet->balance ?? 0 }}</span>
                        </p>
                        <p class="font-semibold text-red-700 mt-1">
                            Previous Wallet Balance: <span class="font-semibold text-red-700">₹ {{ $previouswalletamount ?? 0 }}</span>
                        </p>
                    </div>
                </div>
                ...
                @if(!empty($delivery))
                    @foreach($delivery as $list)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3">{{ $list->amount ?? '0' }}</td>
                            <td class="px-4 py-3">{{ showDate($list->delivery_date) ?? '' }}</td>
                            <td class="px-4 py-3">{{ $list->delivery_status}}</td>
                            <td class="px-4 py-3">{{ $list->quantity ?? '0' }}</td>
                            <td class="px-4 py-3">{{ $list->pack ?? '0' }}</td>
                            <td class="px-4 py-3 flex justify-center gap-4">
                                <!-- Edit -->
                                @if(!empty($list->delivery_status) && $list->delivery_status == 'cancelled')
                                    <button
                                        @click="selectedId = {{ $list->id }}; open = true"
                                        class="bg-[#276221] hover:bg-[#52a447] transition text-white px-2 py-2 rounded-lg shadow">
                                        Revoke
                                    </button>
                                @endif
