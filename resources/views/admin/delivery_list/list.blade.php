<x-layouts.app>
    <div class="p-4">
        <!-- Filter Form -->
        <form method="GET" action="{{ route('lists.delivery_list') }}"
            class="bg-white p-5 rounded-lg shadow flex flex-wrap gap-4 items-end">
            <!-- City Filter -->
            <div>
                <label for="city" class="block text-sm font-medium text-gray-700 mb-1">City</label>
                <select name="city" id="city"
                    class="block w-52 rounded-lg border border-gray-300 p-2 outline-none">
            ...
            <div>
                ...
                <input type="month" name="delivery_month" id="delivery_month"
                    class="block w-52 rounded-lg border border-gray-300 p-2 outline-none"
                    value="{{ request('delivery_month') }}">
            </div>

            <!-- Username Filter -->
            <div>
                <label for="username" class="block text-sm font-medium text-gray-700 mb-1">Username</label>
                <input type="text" name="username" id="username" placeholder="Search by username"
                    class="block w-52 rounded-lg border border-gray-300 p-2 outline-none"
                ...
                <select name="delivery_boy" id="delivery_boy"
                    ...
                    @foreach ($delivery_boy as $boy)
                        <option value="{{ $boy->name }}"
                            {{ request('delivery_boy') == $boy->name ? 'selected' : '' }}>
                            {{ $boy->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex gap-2">
                <button type="submit"
                    class="px-4 py-2 bg-[#ab5f00] text-white rounded-lg hover:bg-[#ab7d00]">Filter</button>
                <a href="{{ route('lists.delivery_list') }}"
                    ...
                    @forelse ($daily_delivery as $list)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <span
                                    class="px-3 py-1 text-xs font-semibold rounded-full {{ $isDelivered ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700' }}">
